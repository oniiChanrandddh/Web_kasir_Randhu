...
$stmt = $conn->prepare("SELECT nama FROM members WHERE id = ?");
$stmt->bind_param("i", $memberId);
$stmt->execute();
$stmt->bind_result($namaMember);
if (!$stmt->fetch()) {
    $stmt->close();
    header("Location: ../dashboardAdmin.php");
    exit();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $conn->query("DELETE FROM members WHERE id=$memberId");
    ...
    <div class="layout">
        <div class="content">
            ...
            <div class="section-edit">
                <p class="warning">Apakah Anda yakin ingin menghapus member berikut?</p>
                <p><b><?= htmlspecialchars($namaMember) ?></b></p>
